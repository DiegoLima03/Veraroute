...
require_once __DIR__ . '/../models/DireccionEntrega.php';
...
class ClienteController extends Controlador
{
    ...
    public function __construct()
    {
        ...
        $this->schedule = new HorarioCliente();
    }

    public function index()
    {
        Autenticacion::requireRole('admin', 'logistica', 'comercial');
        $clients = Autenticacion::isComercial()
            ? $this->client->getAllByComercialIds(Autenticacion::comercialIds())
            : $this->client->getAll();
    ...
    {
        Autenticacion::requireRole('admin', 'logistica');
        $dir = new DireccionEntrega();
        $this->json($dir->getByCliente((int) $clientId));
    }

    /** POST api/clients/{id}/addresses */
    public function storeAddress($clientId)
    {
        Autenticacion::requireRole('admin');
        $data = $this->getInput();
        $dir = new DireccionEntrega();
        $dir->create((int) $clientId, $data);
        $this->json($dir->getByCliente((int) $clientId));
    }

    /** PUT api/clients/{id}/addresses/{addressId} */
    public function updateAddress($clientId, $addressId)
    {
        Autenticacion::requireRole('admin');
        $data = $this->getInput();
        $dir = new DireccionEntrega();
        $dir->update((int) $addressId, $data);
        $this->json($dir->getByCliente((int) $clientId));
    }

    /** DELETE api/clients/{id}/addresses/{addressId} */
    public function destroyAddress($clientId, $addressId)
    {
        Autenticacion::requireRole('admin');
        $dir = new DireccionEntrega();
        $dir->delete((int) $addressId);
        $this->json($dir->getByCliente((int) $clientId));
    }

    /** PUT api/clients/{id}/addresses/{addressId}/principal */
    public function setAddressPrincipal($clientId, $addressId)
    {
        Autenticacion::requireRole('admin');
        $dir = new DireccionEntrega();
        $dir->setPrincipal((int) $addressId);
        $this->json($dir->getByCliente((int) $clientId));
    }
}
