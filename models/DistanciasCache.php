...
class DistanciasCache extends Modelo
{
    ...
    public function get(float $lat1, float $lng1, float $lat2, float $lng2): ?array
    {
        $row = $this->query(
            'SELECT distance_km, duration_s FROM distancias_cache
             WHERE latitud_origen = ? AND longitud_origen = ? AND latitud_destino = ? AND longitud_destino = ?',
    ...
    {
        $this->query(
            'INSERT IGNORE INTO distancias_cache (latitud_origen, longitud_origen, latitud_destino, longitud_destino, distance_km, duration_s)
             VALUES (?, ?, ?, ?, ?, ?)',
