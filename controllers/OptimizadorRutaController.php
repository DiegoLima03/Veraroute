...
require_once __DIR__ . '/../models/PlanRuta.php';
require_once __DIR__ . '/../models/Pedido.php';
require_once __DIR__ . '/../models/Cliente.php';
require_once __DIR__ . '/../models/DistanciasCache.php';
require_once __DIR__ . '/../models/HorarioCliente.php';
require_once __DIR__ . '/../models/Configuracion.php';
require_once __DIR__ . '/../services/OptimizadorRuta.php';

class OptimizadorRutaController extends Controlador
{
    ...
    public function __construct()
    {
        ...
        $this->routePlan       = new PlanRuta();
        $this->orderModel      = new Pedido();
        $this->clientModel     = new Cliente();
        $this->distCache       = new DistanciasCache();
        $this->scheduleModel   = new HorarioCliente();
        $this->settingModel    = new Configuracion();
    }
    ...
    public function optimize()
    {
        ...
        foreach ($ordersMap as $clientId => $orderData) {
            ...
            if (isset($clientMap[$cid]) && $clientMap[$cid]['active']) {
                ...
                // L7 activado 02-may-2026: pasamos el cliente para que calculateUnloadTime
                // use el modelo parametrizado (parada_min por zona + espera_min por negocio).
                // Sin arrivalMin → multiplicador de franja = 1.0 (la activación dinámica
                // por franja requiere recalcular durante el bucle NN, pendiente futuro).
                $unloadTime = $this->routePlan->calculateUnloadTime($orderData['id'], $this->baseUnloadMin, $clientMap[$cid]);
                $eligible[] = [
                    'client'        => $clientMap[$cid],
                    'id_pedido'      => $orderData['id'],
                    'load'          => $load,
                    'unload_min'    => $unloadTime,
                    'id_delegacion' => $clientMap[$cid]['id_delegacion'] ? (int) $clientMap[$cid]['id_delegacion'] : null,
                ];
            }
        }
        ...
        // 3. Asignar clientes a delegaciones (por id_delegacion o por cercania)
        $delegationIndex = [];
        foreach ($delegations as $d) {
            $delegationIndex[(int) $d['id']] = $d;
        }

        $clusters = []; // id_delegacion => [eligible entries]
        foreach ($eligible as $e) {
            $assignedDelegation = null;
            if ($e['id_delegacion'] && isset($delegationIndex[$e['id_delegacion']])) {
                $assignedDelegation = $e['id_delegacion'];
            } else {
        ...
        foreach ($clusters as $delegationId => $entries) {
            ...
            if (!count($delegationVehicles)) {
                foreach ($entries as $e) {
                    $unassigned[] = ['id_cliente' => $e['client']['id'], 'name' => $e['client']['name'], 'reason' => 'Sin vehiculo en delegacion ' . $delegation['name']];
                }
                continue;
            }
            ...
            foreach ($entries as $e) {
                ...
                if (!$found) {
                    $unassigned[] = ['id_cliente' => $e['client']['id'], 'name' => $e['client']['name'], 'reason' => 'Excede capacidad'];
                }
            }
        }
        ...
        $this->json([
            ...
            'settings'   => [
                'almuerzo_duracion_min' => $this->lunchDuration,
                'almuerzo_hora_min'     => sprintf('%02d:%02d', floor($this->lunchEarliest / 60), $this->lunchEarliest % 60),
                'almuerzo_hora_max'       => sprintf('%02d:%02d', floor($this->lunchLatest / 60), $this->lunchLatest % 60),
                'descarga_min_base'    => $this->baseUnloadMin,
            ],
        ]);
    }
    ...
    /**
     * Wrapper L6: para flota pequeña (≤ 8 vehículos) y carga moderada (≤ 12 entries)
     * intenta búsqueda exhaustiva con poda; si supera el cap de nodos o el tamaño
     * excede el umbral, cae a FFD. Garantiza máximo de asignados sin penalizar
     * casos grandes donde FFD ya da soluciones razonables.
     */
    private function assignToVehicles(array $entries, array $vehicles): array
    {
        $ffd = $this->assignToVehiclesFFD($entries, $vehicles);
        $ffdAssigned = array_sum(array_map('count', $ffd));

        if (count($vehicles) <= 8 && count($entries) <= 12) {
            $best = $ffd;
            $bestAssigned = $ffdAssigned;
            $remaining = array_map(fn($v) => [
                'items' => 0, 'weight' => 0, 'volume' => 0,
                'max_items'  => (int) ($v['max_items'] ?? 0),
                'max_weight' => (float) ($v['max_weight_kg'] ?? 0),
                'max_volume' => (float) ($v['max_volume_m3'] ?? 0),
            ], $vehicles);
            $assignments = array_fill(0, count($vehicles), []);
            $nodes = 0;
            $this->backtrackAssign($entries, 0, $remaining, $assignments, 0, $best, $bestAssigned, $nodes, 100000);
            return $best;
        }
    ...
    /** L6: backtracking con poda. Mejora $best/$bestAssigned in-place. Aborta si supera $maxNodes. */
    private function backtrackAssign(
        array $entries, int $idx, array $remaining, array $assignments,
        int $assigned, array &$best, int &$bestAssigned, int &$nodes, int $maxNodes
    ): void {
        ...
        // Poda: si asignando todos los restantes no superaríamos bestAssigned, abortar
        if ($assigned + (count($entries) - $idx) <= $bestAssigned) return;

        $e = $entries[$idx];
        foreach ($remaining as $vi => $r) {
            $curItems  = $r['items']  + $e['load']['items'];
            $curWeight = $r['weight'] + $e['load']['peso_kg'];
            $curVolume = $r['volume'] + $e['load']['volume_m3'];
            if ($r['max_items']  && $curItems  > $r['max_items'])  continue;
            if ($r['max_weight'] && $curWeight > $r['max_weight']) continue;
            if ($r['max_volume'] && $curVolume > $r['max_volume']) continue;

            $newRemaining = $remaining;
            $newRemaining[$vi]['items']  = $curItems;
            $newRemaining[$vi]['weight'] = $curWeight;
            $newRemaining[$vi]['volume'] = $curVolume;
            $newAssignments = $assignments;
            $newAssignments[$vi][] = $e;

            $this->backtrackAssign($entries, $idx + 1, $newRemaining, $newAssignments, $assigned + 1, $best, $bestAssigned, $nodes, $maxNodes);
        }

        // Probar dejar este entry sin asignar (queda como unassigned)
        $this->backtrackAssign($entries, $idx + 1, $remaining, $assignments, $assigned, $best, $bestAssigned, $nodes, $maxNodes);
    }

    /** FFD original: ordena entries por items descendente y asigna al vehículo con menos carga actual. */
    private function assignToVehiclesFFD(array $entries, array $vehicles): array
    {
        ...
        foreach ($entries as $e) {
            ...
            foreach ($vehicles as $vi => $v) {
                ...
                $curWeight = $remaining[$vi]['weight'] + $e['load']['peso_kg'];
            ...
            if ($bestVi !== null) {
                ...
                $remaining[$bestVi]['weight'] += $e['load']['peso_kg'];
    ...
    private function loadSettings(): void
    {
        ...
        $this->lunchDuration = (int) ($s['almuerzo_duracion_min'] ?? 60);
        $this->lunchEarliest = $this->timeToMin($s['almuerzo_hora_min'] ?? '12:00');
        $this->lunchLatest   = $this->timeToMin($s['almuerzo_hora_max'] ?? '15:30');
        $this->baseUnloadMin = (float) ($s['descarga_min_base'] ?? 5.0);
    }
    ...
    private function optimizeVehicleRoute(array $delegation, array $entries): array
    {
        ...
        // L3: multi-start. Tres estrategias de NN + 2-opt; gana la que minimiza
        // unassigned (empate: menos km). Garantiza que un orden globalmente mejor
        // no quede oculto por el sesgo greedy de una única estrategia.
        $resultados = [
            'lookahead' => $this->runNNStrategy('lookahead', $entries, $dist, $dur, $delegationOpenMin),
            'edf'       => $this->runNNStrategy('edf',       $entries, $dist, $dur, $delegationOpenMin),
            'ratio'     => $this->runNNStrategy('ratio',     $entries, $dist, $dur, $delegationOpenMin),
        ];
        uasort($resultados, function ($a, $b) {
            if ($a['numUnassigned'] !== $b['numUnassigned']) return $a['numUnassigned'] - $b['numUnassigned'];
            return $a['kmTotal'] <=> $b['kmTotal'];
        });
        $ganador = reset($resultados);
        $order = $ganador['order'];
        ...
        foreach ($order as $idx => $ei) {
            ...
            $route[] = [
                'id_cliente'   => (int) $entries[$ei]['client']['id'],
                'id_pedido'    => $entries[$ei]['id_pedido'],
    ...
    /** 2-opt delegado a OptimizadorRuta */
    private function twoOpt(array $order, array $distMatrix, array $entries, array $dur, int $startMin): array
    {
        // L2: callback que verifica si un orden candidato cumple todas las ventanas
        // horarias. Reutiliza simulateDeparture sin almuerzo (lunchAfterStop=null);
        // el almuerzo se posiciona después en findBestLunchPosition.
        $isFeasible = function (array $candidateOrder) use ($entries, $dur, $startMin): bool {
            return $this->simulateDeparture($candidateOrder, $entries, $dur, $startMin, null);
        };
        return (new OptimizadorRuta())->twoOptDistanceMatrixConstrained($order, $distMatrix, $isFeasible);
    }
    ...
    /**
     * L3: ejecuta una estrategia NN (lookahead/edf/ratio) seguida de 2-opt
     * constrained. Devuelve order, kmTotal y numUnassigned. Reutilizado por el
     * multi-start en optimizeVehicleRoute.
     */
    private function runNNStrategy(string $strategy, array $entries, array $dist, array $dur, int $startMin): array
    {
        $n = count($entries);
        ...
        for ($step = 0; $step < $n; $step++) {
            ...
            for ($ei = 0; $ei < $n; $ei++) {
                ...
                $pi = $ei + 1;
                $arrivalMin = $t + ($dur[$curIdx][$pi]) / 60;
                if (!$this->canVisitAt($arrivalMin, $entries[$ei]['client'])) continue;
                $candidatos[] = ['ei' => $ei, 'd' => $dist[$curIdx][$pi], 'arrival' => $arrivalMin];
            }
            if (empty($candidatos)) break;

            $bestEi = $this->pickNextStop($strategy, $candidatos, $entries, $dist, $dur, $curIdx, $t, $visited);
        ...
        $order = $this->twoOpt($order, $dist, $entries, $dur, $startMin);

        $kmTotal = 0.0;
        $prev = 0;
        foreach ($order as $ei) {
            $pi = $ei + 1;
            $kmTotal += $dist[$prev][$pi];
            $prev = $pi;
        }
        $kmTotal += $dist[$prev][0];
        ...
        return [
            'order' => $order,
            'kmTotal' => $kmTotal,
            'numUnassigned' => $n - count($order),
        ];
    }

    /**
     * L3: selecciona el próximo cliente según la estrategia. Estrategias:
     *  - 'lookahead' (L1): marcar urgentes (no visitables si fueran último), elegir más cercano del pool.
     *  - 'edf' (Earliest Deadline First): cierre más temprano; en empate, más cercano.
     *  - 'ratio' (carga/ventana): mayor unload_min/ancho_ventana = más urgente; en empate, más cercano.
     */
    private function pickNextStop(string $strategy, array $candidatos, array $entries, array $dist, array $dur, int $curIdx, float $t, array $visited): ?int
    {
        if ($strategy === 'lookahead') {
            $urgentes = [];
            foreach ($candidatos as $cand) {
                if (!$this->wouldStillBeFeasibleIfLast($cand['ei'], $visited, $entries, $dist, $dur, $curIdx, $t)) {
                    $urgentes[] = $cand;
                }
            }
            $pool = !empty($urgentes) ? $urgentes : $candidatos;
            $best = null; $bestD = PHP_FLOAT_MAX;
            foreach ($pool as $c) if ($c['d'] < $bestD) { $bestD = $c['d']; $best = $c['ei']; }
            return $best;
        }

        if ($strategy === 'edf') {
            $best = null; $bestClose = PHP_INT_MAX; $bestD = PHP_FLOAT_MAX;
            foreach ($candidatos as $c) {
                $earliestClose = PHP_INT_MAX;
                foreach ($this->getClientWindows($entries[$c['ei']]['client']) as $w) {
                    $cl = $this->timeToMin($w['close_time']);
                    if ($c['arrival'] <= $cl) $earliestClose = min($earliestClose, $cl);
                }
                if ($earliestClose < $bestClose || ($earliestClose === $bestClose && $c['d'] < $bestD)) {
                    $bestClose = $earliestClose; $bestD = $c['d']; $best = $c['ei'];
                }
            }
            return $best;
        }
        ...
        if ($strategy === 'ratio') {
            $best = null; $bestRatio = -1.0; $bestD = PHP_FLOAT_MAX;
            foreach ($candidatos as $c) {
                $ancho = 0;
                foreach ($this->getClientWindows($entries[$c['ei']]['client']) as $w) {
                    $cl = $this->timeToMin($w['close_time']);
                    if ($c['arrival'] <= $cl) {
                        $op = $this->timeToMin($w['open_time']);
                        $ancho = max($ancho, $cl - $op);
                        break;
                    }
                }
                $unload = (float) ($entries[$c['ei']]['unload_min'] ?? 5);
                $ratio = $ancho > 0 ? ($unload / $ancho) : 1.0;
                if ($ratio > $bestRatio || (abs($ratio - $bestRatio) < 1e-9 && $c['d'] < $bestD)) {
                    $bestRatio = $ratio; $bestD = $c['d']; $best = $c['ei'];
                }
            }
            return $best;
        }
    ...
    /**
     * L1 look-ahead: simula si $targetEi seguiria siendo visitable dejandolo
     * para el ultimo (NN puro sobre los demas no visitados). Devuelve true si
     * llega dentro de alguna de sus ventanas, false si no.
     */
    private function wouldStillBeFeasibleIfLast(
        int $targetEi,
        array $visited,
        array $entries,
        array $dist,
        ...
        $localVisited[$targetEi] = true; // reservado para el final
        ...
        while (true) {
            ...
            for ($ei = 0; $ei < $n; $ei++) {
                if ($localVisited[$ei]) continue;
                $pi = $ei + 1;
                $arrivalMin = $tt + ($dur[$cur][$pi]) / 60;
                if (!$this->canVisitAt($arrivalMin, $entries[$ei]['client'])) continue;
                if ($dist[$cur][$pi] < $bestDist) {
                    $bestDist = $dist[$cur][$pi];
                    $bestEi = $ei;
                }
            }
            if ($bestEi === null) break;
            $localVisited[$bestEi] = true;
            $pi = $bestEi + 1;
            $tt += ($dur[$cur][$pi]) / 60;
            $tt = $this->adjustToOpenTime($tt, $entries[$bestEi]['client']);
            $tt += $entries[$bestEi]['unload_min'];
            $cur = $pi;
        }

        $piTarget = $targetEi + 1;
        $arrivalTarget = $tt + ($dur[$cur][$piTarget]) / 60;
        return $this->canVisitAt($arrivalTarget, $entries[$targetEi]['client']);
    }
