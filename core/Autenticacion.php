...
class Autenticacion
{
    ...
    {
        $u = self::currentUser();
        return $u && $u['role'] === 'comercial' ? ($u['id_comercial'] ?? null) : null;
    }
    ...
    public static function comercialIds(): array
    {
        ...
        $ids = [];
        if (!empty($u['id_comercial'])) {
            $ids[] = (int) $u['id_comercial'];
        }

        try {
            $db = Database::connect();
            $stmt = $db->prepare('SELECT id_comercial FROM usuario_comerciales WHERE id_usuario = ?');
            $stmt->execute([$userId]);

            foreach ($stmt->fetchAll() as $row) {
                if (!empty($row['id_comercial'])) {
                    $ids[] = (int) $row['id_comercial'];
                }
            }
        } catch (\Throwable $e) {
            // Si la tabla intermedia aun no existe, mantenemos la compatibilidad con id_comercial.
        }
    ...
    public static function login(array $userData)
    {
        ...
        $_SESSION['user'] = [
            ...
            'role'         => $userData['role'],
            'id_comercial' => $userData['id_comercial'] ? (int) $userData['id_comercial'] : null,
        ];
    }
